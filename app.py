...
def draw_card(row):
    with st.container(border=True):
        ...
        with st.expander("View Trade Details"):
            ...
            <div style="margin-bottom: 10px;"><div style="font-size: 0.6rem; opacity: 0.7; font-weight: 700; text-transform: uppercase;">Live Price</div><div style="font-size: 1.3rem; font-weight: 800; color: var(--text-color); line-height: 1.1;">₹{live_p_val}</div></div>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if not history_df.empty:
            ...
            def calculate_closed_pnl(row):
                e_val = safe_float(row.get('Entry Price', 0))
                l_val = safe_float(row.get('Live Price', 0))
                if e_val > 0: return ((l_val - e_val) / e_val) * 100
                return 0.0
